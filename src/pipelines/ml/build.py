...
def _construct_report(config, now, vocab_size, model):
    report = {}
...
def main():
    ...
    # Input
    documents_fp = path.join(ASSETS_FP, "datasets", "datamart", "train.csv")
    ...
    model_fp = path.join(
        ASSETS_FP, "models", f"{workflow}-{experiment}-{model_timestamp}.pkl"
    )
    model_json_fp = path.join(
        ASSETS_FP, "models", f"{workflow}profile-{experiment}-{model_timestamp}.json"
    )
    ...
    j = _construct_report(config, now, vocab_size, model)
